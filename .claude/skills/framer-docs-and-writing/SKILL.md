...
| `README.md` | **RESOLVED 2026-08-19** — post-v2.1.0 drift fixed: Shader bullet updated to 12 looks (v2.1.0 added `roughBorder`, `filmGrain`, `bwFilm` as `ShaderStyle` cases — not new `CompositionLayer` cases, so "12 layer types" was never wrong), a "B&W Film Stack (v2.1.0)" feature section added, and the dither count corrected 17 → 16 (`DitherAlgorithm` has 16 cases; the miscount dated to the v2.0.0 rewrite). Everything else re-verified 2026-08-19 against v2.1.0: 12 user-facing GPU effects ✓, 168 overlays ✓ (170 files in `assets/textures/` minus the 2 ASCII LUT atlas PNGs — the 168 LFS objects ARE the overlays), preset paths/list ✓, install steps ✓. (History: fully rewritten 2026-07-09 in the v2.0.0 release PR.) | `Sources/FramerCore/Models/CompositionLayer.swift` (`ShaderStyle`, `DitherAlgorithm`); CHANGELOG `[2.1.0]` for what shipped. |
