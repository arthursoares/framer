...
Caution: bucket (`.gpuEffect`) renderer behavior changed with PR #12 (merged 2026-07-09, commit `20d5775` "drop CPU fallbacks where the GPU path is complete"): Glitch/EdgeField buckets are now GPU-only and THROW instead of silently falling back (the old `// fall through to CPU` in GlitchRenderer is gone — the deleted CPU loops interpreted parameters differently, so a fallback render looked visibly wrong). TextCell/PrintSampling keep CPU loops ONLY for legacy hidden variants (`.ascii`, `.halftone`, `.dithering` bucket forms) that have no GPU entry. For buckets, run `swift test --filter EffectGPUParityTests` — the suite self-skips with `XCTSkip("Metal device unavailable…")` when there's no GPU library.
