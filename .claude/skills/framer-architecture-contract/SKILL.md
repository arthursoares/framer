...
| PresetStore DELETES preset files it cannot decode | PresetStore.swift:58-64 (`try? FileManager.default.removeItem(at: url)` in the `catch`) | A Codable regression in `Preset`/`ProcessingConfig` silently destroys user presets on next launch. Test Codable changes against saved-preset fixtures first (framer-validation-and-qa) |
...
| Docs of record are partially stale (e.g. docs/gpu-effects-parameter-matrix.md predates later shader wiring; CLAUDE.md points at `.ai-assistant/` dirs that do not exist) | staleness ledger: framer-docs-and-writing | Do not treat prose docs as ground truth for shader capabilities; `GPUEffectKind` capability flags (Sources/FramerCore/Effects/Models/GPUEffectKind.swift:81-124) + the `.metal` sources are ground truth |
