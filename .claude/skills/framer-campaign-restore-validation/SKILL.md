...
**If you see skips** (`XCTSkip`) → you are on a machine without a Metal device or without LFS-hydrated fixtures; the effect render suites self-skip — 69 Metal-dependent tests as of the 2026-07-09 CPU-path retirement; only the Riemersma render/routing tests still render without Metal (skip arithmetic is owned by `framer-validation-and-qa`). The campaign's later phases require a real Mac with a GPU, so relocate before continuing (see `framer-build-and-env`).
